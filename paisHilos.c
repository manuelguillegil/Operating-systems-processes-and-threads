...
void accionesConservadas(FILE* fp, char* temp, char* newName){
    size_t len = 0;
    char* line;
    FILE* f = fopen(temp, "w");
    srand(time(0));
    ...
    while(getline(&line, &len, fp)!=-1){                                                           //Itera por todas las lineas del archivo
        if(strlen(line) > 2 && strchr(line, ':') == NULL && rand()%101 > PorcentajeExitoEjec){     //Si encuentra una accion y decide quitarla del plan
...
{ 
    ...
    while(day-1<daysMax){                                          
        ...
        if(censurado == TRUE){                                            //Si el presidente fue censurado
            ...
            accionesConservadas(fp, "tempEjec.acc", direccionEjec);       //Se revisa que acciones conserva y cuales Desecha
